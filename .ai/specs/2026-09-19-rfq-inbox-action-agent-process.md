...
- **Leading indicators:** a successful `property_documents.pdf_intake` run, one `property_documents.room_measurements` run per rendered PDF page, and one completed `rfq_intake.analyze-document` command that ran catalog matching and page fan-out concurrently.
- **Baseline:** zero — the link does not exist; agents are started by hand.
- **Market / product reference:** [Camunda's parallel-gateway documentation](https://docs.camunda.io/docs/components/modeler/bpmn/parallel-gateways) describes static concurrent branches. The installed runtime stalls when an `INVOKE_AGENT` signal enters such a fork, so this hackathon slice runs the two independent commands concurrently inside one app-owned workflow command until that framework defect is repaired.
...
- **REQ-008** — Catalog matching and page measurements begin only after PDF intake succeeds, execute concurrently inside `rfq_intake.analyze-document`, and preserve tenant, organization, actor, workflow instance, source run, and page-artifact correlation for each measurement run.
...
4. A **code-defined workflow** that invokes `pdf_intake`, then invokes `rfq_intake.analyze-document`; that app command concurrently starts catalog matching and per-page room measurement fan-out.
...
| Run catalog matching and page measurements together inside `rfq_intake.analyze-document` | The installed runtime opens static fork tokens after an `INVOKE_AGENT` signal but leaves them dormant; the app command uses `Promise.allSettled` to start the two independent commands concurrently and waits for both | Keep `PARALLEL_FORK` / `PARALLEL_JOIN` | The graph visually represents the intended shape, but agents never run after the fork in the installed runtime |
