...
- **REQ-008** — Catalog matching begins only after PDF intake succeeds; page measurements begin only after catalog matching completes. Every step preserves tenant, organization, actor, workflow instance, source run, and page-artifact correlation.
...
4. A **code-defined workflow** that invokes `pdf_intake`, then catalog matching, then per-page room measurement fan-out.
...
| Run catalog matching before page measurements | The installed runtime opens static fork tokens after an `INVOKE_AGENT` signal but leaves them dormant. A linear pipeline preserves durable visible progress and avoids that runtime defect | Keep `PARALLEL_FORK` / `PARALLEL_JOIN` or app-owned concurrent command fan-out | The fork stalls; the concurrent command hides progress inside one workflow activity |
...
| Analysis ordering | App-owned workflow | `pdf_intake` → catalog match → page measurement fan-out | Each stage must finish before the next begins; fan-out remains inside the scoped measurement command |
...
- **Exit gate:** the generated workflow definition validates with linear post-intake stages; TEST-016 proves every rendered page reaches a one-file measurement run; a manual run of TEST-005 produces manifests and page measurement runs.
...
| Every workflow completes end to end without a catch-all phase | pass | Phase 1 completes through the linear PDF intake, catalog match, and measurement pipeline |
| Platform-native reuse and extension points chosen before custom code | pass | Reuse map: artifact promotion, agent runtime, and code workflow; static fork/join is deliberately not used pending issue #6280 |
