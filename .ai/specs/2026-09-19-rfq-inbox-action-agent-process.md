...
- **REQ-002** — The same acceptance starts, post-commit and without blocking or failing the action, a workflow that runs `property_documents.pdf_intake` over the e-mail's PDF, then `property_documents.room_dimensions` over the extracted floor plan, then `property_documents.catalog_matcher` over the brief's requirement text.
...
- More than one PDF, more than one floor plan, and more than one catalog-matcher call per RFQ. Phase 1 is deliberately linear — see the shortcut decision below.
...
| Phase 1 is a linear four-node chain over **one** PDF, **one** floor plan and **one** concatenated requirement text | The engine has no dynamic fan-out: `PARALLEL_FORK` opens one branch per outgoing static transition (`workflows/lib/parallel-handler.ts:120`), nested forks are rejected (`step-handler.ts:493`), `SUB_WORKFLOW` starts exactly one child (`step-handler.ts:1120`), and no `forEach` construct exists. The agents' own tools refuse more than one input (`ai-tools.ts:600`, `ai-tools.ts:576`) | Fan out per plan and per requirement; or push iteration into an orchestrating agent with sub-agents | Both are real and both are Phase 2. Sub-agent fan-out additionally puts the iteration count in the model's hands, which is where items get silently dropped. `HACK(hackathon)` at the seam |
...
| Analysed plan | The first entry of `floor-plans.json`, promoted to an attachment on the quote | `floor-plans.json` produced by `pdf_intake` | No plan in the manifest → the chain stops after `pdf_intake`, recorded on the run |
| Matcher input | The brief's requirement texts, concatenated into one string for a single `catalog_matcher` call | `brief.json` | Empty requirements → the matcher step is skipped |
...
e-mail (PDF)
  ...
  -> workflows event-trigger (wildcard subscriber, code triggers included)
      -> workflow `rfq_intake.analysis`:
           START
           -> INVOKE_AGENT  property_documents.pdf_intake      -> brief.json, floor-plans.json, plan PNGs
           -> AUTOMATED     rfq_intake.plan.promote            -> first plan PNG becomes an attachment on the quote
           -> INVOKE_AGENT  property_documents.room_dimensions -> rooms + dimensions
           -> INVOKE_AGENT  property_documents.catalog_matcher -> catalog matches for the brief's requirements
           END
```
...
| TEST-011 | unit | Agent run whose `floor-plans.json` names two plans | Execute `rfq_intake.plan.promote` | The first plan's PNG is promoted once; a second execution returns the same `attachmentId` | REQ-002 |
...
- **Deliverables:** `src/modules/rfq_intake/{index.ts,inbox-actions.ts,events.ts,subscribers/startRfqAnalysis.ts,workflows.ts}`; `rfq_intake` added to `src/modules.ts` **after** `sales`; i18n label change in `src/i18n/pl.json` and its English counterpart; the denylist filter in `src/bootstrap-common.ts`.
- **Requirements closed:** REQ-001 … REQ-006
- **Tests:** TEST-001 … TEST-004 and TEST-006 … TEST-011 automated, TEST-005 manual
...
6. Add `workflows.ts` with `rfq_intake.analysis` — the four-node linear chain above, embedded event trigger on `rfq_intake.rfq.created` with a 1:1 `contextMapping`. Each `INVOKE_AGENT` activity declares an `outputMapping` so the next step reads its predecessor's result from the workflow context. Cast the activity type where `@open-mercato/shared`'s stale `ActivityType` union rejects `INVOKE_AGENT`, with a comment naming the reason. The same file registers `rfq_intake.plan.promote` through `registerWorkflowSafeCommands` — a command a workflow calls must be declared workflow-safe, and a new one is **not** `defaultEnabled`, so the demo tenant has to enable it in settings. TEST-004, TEST-011.
7. Filter `registerCodeWorkflows` in `src/bootstrap-common.ts` against a named denylist — `workflows.simple-approval`, `workflows.checkout-demo`, `sales.order-approval` — with a comment stating that the first two are shipped demos and the third is a deliberate product removal. TEST-010.
8. Run TEST-005 by hand end to end; record what actually happened, including anything stubbed.
...
- **Fan-out.** One workflow instance per unit of work, chained by events: `pdf_intake` ends by emitting one event per floor plan, each starting a `room_dimensions` instance, which in turn emits one event per requirement for `catalog_matcher`. The iteration lives in our own commands, where the count is deterministic and testable; every agent call stays an `INVOKE_AGENT` step with its own trace and disposition. A fourth agent is then a new event plus a new code workflow, touching none of the existing three.
...
| Only the first floor plan and one concatenated requirement text are analysed | A multi-plan PDF is read partially; the demo understates what the chain could find | `HACK(hackathon)` at the workflow node, named in the non-goals; Phase 2 replaces it | Accepted for this slice |
| `rfq_intake.plan.promote` is not `defaultEnabled` as a workflow-safe command | The chain stops at step 2 on a tenant that never opened the workflow-command settings | Part of TEST-005's manual checklist | Accepted — grandfathering a new command is explicitly discouraged upstream |
