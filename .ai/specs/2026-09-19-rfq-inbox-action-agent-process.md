...
e-mail (PDF)
  ...
  -> operator accepts            -> sales execute  -> SalesQuote (draft)  [= the RFQ]
...
3. Operator accepts. The draft quote is created by the installed `sales` path; the action shows executed.
4. Post-commit, the analysis starts. The operator's screen does not wait on it.
5. Failure cases: no PDF attachment → no analysis starts, the quote still exists, and the reason is logged rather than silently swallowed; the agent failing later fails its own run, never the executed action.
...
- **Tests:** TEST-001 … TEST-004 automated, TEST-005 manual
...
4. Add `events.ts` declaring `rfq_intake.rfq.created` and the subscriber on `inbox_ops.action.executed`, emitting post-commit with the attachment-shaped payload. TEST-001, TEST-002.
5. Add `workflows.ts` with `rfq_intake.analysis`: `START → AUTOMATED[INVOKE_AGENT(property_documents.pdf_intake)] → END`, embedded event trigger on `rfq_intake.rfq.created` with a 1:1 `contextMapping`. Cast the activity type where `@open-mercato/shared`'s stale `ActivityType` union rejects `INVOKE_AGENT`, with a comment naming the reason. TEST-004.
6. Run TEST-005 by hand end to end; record what actually happened, including anything stubbed.
