...
- **In THIS app, `overrides.routes.api` in `src/modules.ts` cannot carry a server-side handler.** `src/components/ClientBootstrap.tsx:66` does `import('@/modules')` in the BROWSER to apply the `widgets`/`notifications` overrides, so every module `modules.ts` imports becomes client-reachable. Naming an installed API route's replacement there pulls that route — and `server-only` with it — into the client graph, and `yarn build` fails with a Turbopack import trace reading `modules.ts -> ClientBootstrap.tsx -> layout.tsx`. A lazy `import()` inside the handler does not help; the edge is still traced. Register such a handler through the programmatic tier instead — `applyApiRouteOverrides({...})` in `src/bootstrap-common.ts`, which only `bootstrap.ts` and `bootstrap-api.ts` import — and keep it BEFORE `registerApiRouteManifests`, which reads the store once. Worked example: `src/lib/publicQuoteAcceptOrigin.ts`. Incident: 2026-09-19, 48 Turbopack errors.
...
Canonical example source: [`src/modules.ts`](../../../../src/modules.ts) is the app registry, not the `example` module. **Correction, 2026-09-19: in this repo it carries NO `entry.overrides` examples at all** — the upstream sentence claiming typed, inactive examples for every wired domain is false here, so read the shipped types in `@open-mercato/shared/src/modules/overrides.ts` (`ModuleOverrides`, `ApiRouteOverridesMap`) for the exact shape instead.
