...
- **In THIS app, `overrides.routes.api` in `src/modules.ts` cannot carry a server-side handler.** `src/components/ClientBootstrap.tsx:66` calls `import('@/modules')` in the BROWSER, so every module that `modules.ts` imports becomes client-reachable. Naming an installed API route's replacement there pulls that route into the client graph, and `yarn build` fails with a Turbopack trace that reads `modules.ts -> ClientBootstrap.tsx -> layout.tsx`. A lazy `import()` in the handler does not help, because Turbopack still traces the edge. Use the programmatic tier instead: call `applyApiRouteOverrides({...})` in `src/bootstrap-common.ts`, which only `bootstrap.ts` and `bootstrap-api.ts` import. Keep the call BEFORE `registerApiRouteManifests`, which reads the store one time. Worked example: `src/lib/publicQuoteAcceptOrigin.ts` (incident 2026-09-19, 48 Turbopack errors).
...
Canonical example source: [`src/modules.ts`](../../../../src/modules.ts) is the app registry, not the `example` module. **Correction, 2026-09-19: in this repo it carries NO `entry.overrides` examples at all.** The upstream claim of typed, inactive examples for every wired domain is false here. Read the shipped types in `@open-mercato/shared/src/modules/overrides.ts` instead: `ModuleOverrides` and `ApiRouteOverridesMap` give the exact shape.
