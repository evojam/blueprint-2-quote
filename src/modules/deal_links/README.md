...
Row labels in both tabs come from one batched `?ids=` request per source
(`lib/link-labels.ts`, shared by both widgets); `makeCrudRoute` supports that param on
every route involved, so nothing upstream was changed. A lookup that fails leaves the
row visible with a shortened id and a "not found" marker instead of hiding it.

Each tab's picker is always visible, whether or not the record already has linked
counterparts, and is visible to anyone who can see the tab; without
`customers.deals.manage` the link attempt returns 403, surfaced inline.
...
`deal_id` nor `document_id` is uniquely indexed. Its picker searches deals by title or
description (`lib/deal-options.ts`, backed by `customers/deals`).

## Known gaps

- **No delete.** Neither the module nor either tab can remove a link. A wrong row
...
- **Encrypted tenants cannot search deals in either picker.** The deals route
  collapses `search` to no matches when tenant data encryption is on, so both
  the deal-side document picker and the document-side deal picker fall back to
  the unfiltered first page and typing narrows it to nothing.
- **Ids are validated, not verified.** `dealId`/`documentId` on the create
  ...
  becomes a dead row in one of the two tabs today, with no cleanup path — the
  module ships no delete route.
...
- **No client-side permission gate on either tab's picker.** The framework has no hook
  for it, and a widget-level `features` gate would hide the link list as well.
