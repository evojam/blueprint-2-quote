...
const VISION_PROMPT = [
  ...
  'When a needed linear wall dimension has neither a printed value nor valid calibration, method estimated is required for the POC. Choose a realistic positive value from visible room proportions; it may be used even when no source dimension or scale exists. Use unit cm, unitSource label, sourceText null, empty evidence, calibrationId null, positive confidence, and a nonempty estimationReason that names the visual basis or states that the drawing has no usable scale. Never represent an estimate as printed or scale_derived.',
  'Use nullable and empty forms only for data that cannot be sensibly estimated. Never invent printed evidence, calibration, source text, or a claim that an estimate is measured.',
